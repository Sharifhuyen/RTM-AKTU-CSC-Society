...
const MyBlogs = () => {
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 py-8 relative">
            {toast && (
                <div className={`fixed top-6 right-6 z-50 w-fit px-4 py-3 rounded shadow-lg text-white transition-all duration-500 ${toast.type === "error" ? "bg-red-600" : "bg-green-600"}`}>{toast.message}</div>
            )}
                        ...
                        >
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                                <div className="flex-1">
                                    ...
                                    <span className={`text-xs mt-1 inline-block px-2 py-0.5 rounded-full font-semibold ${blog.status === "Approved" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
                                        {blog.status}
                                    </span>
                                </div>
